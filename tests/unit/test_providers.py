...
async def test_embedding_adapter_preserves_order_and_dimension() -> None:
    provider = MLXEmbeddingProvider(dimension=2, batch_size=1, loader=lambda _: FakeModel())
...
async def test_missing_optional_provider_degrades() -> None:
    provider = MLXEmbeddingProvider(loader=lambda _: (_ for _ in ()).throw(ImportError("missing")))
