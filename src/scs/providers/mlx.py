...
class MLXEmbeddingProvider:
    ...
    def _load(self) -> _EncodingModel:
        ...
        try:
            if self._loader is not None:
                ...
                from mlx_embedding_models import from_registry

                self._model = from_registry(self._model_name)
    ...
    async def embed_documents(self, texts: Sequence[str]) -> list[list[float]]:
        ...
        async with self._lock:
            ...
            for offset in range(0, len(texts), self._batch_size):
                batch = [f"search_document: {text}" for text in texts[offset : offset + self._batch_size]]
    ...
    async def embed_documents_as(self, task: str, texts: Sequence[str]) -> list[list[float]]:
        """Embed texts using an explicit Nomic task prefix."""

        async with self._lock:
            model = await asyncio.to_thread(self._load)
            encoded = await asyncio.to_thread(model.encode, [f"{task}: {text}" for text in texts])
